...
def cadastrarDadosMaquina(PERC_CPU, PERC_RAM, DISCO, MOMENTO, SERIAL_ID):
    # Fazendo a chamada da Procedure
    sql = f"CALL stg_cadastrarDadosMaquina({PERC_CPU}, {PERC_RAM}, {DISCO}, '{MOMENTO}', '{SERIAL_ID}');"
    con.cursor().execute(sql)
...
def captarDados(DELAY, MAC_ADDRESS, SERIAL_ID):
    ...
    cadastrarDadosMaquina(PERC_CPU, PERC_RAM, DISCO, MOMENTO, SERIAL_ID)
